...
impl Chunk {
    ...
    pub fn new(chunk_type: ChunkType, data: Vec<u8>) -> Self {
        ...
        let crc = CRC_ALGO.checksum(&([&chunk_type.bytes(), data.as_slice()].concat()));
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        let bytes: Vec<u8> = length_bytes
            .into_iter()
            .chain(chunk_type_bytes)
            .chain(self.data.iter().cloned())
            .chain(checksum_bytes)
            .collect();
        bytes
    }
}
...
impl TryFrom<&[u8]> for Chunk {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        let crc_test = CRC_ALGO.checksum(&([&chunk_type.bytes(), data.as_slice()].concat()));
        if crc != crc_test {
